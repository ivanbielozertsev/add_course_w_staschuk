...
console.log(a === b) // false (Почему?) потому что разніе ссілки на блоки памяти

const c = [2, 1, 3]
const d = [1, 2, 3, 4]

function areArraysEqual(firstArray, secondArray) {
    if(firstArray.length === secondArray.length && firstArray.every((item, index) => item === secondArray[index])) {
        return true
    }
    return false
}
